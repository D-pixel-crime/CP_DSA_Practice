...
int euclideanDist(pair<int, int> a, pair<int, int> b)
{
    auto [x1, y1] = a;
    auto [x2, y2] = b;
...
void solve()
{
    ...
    if ((a.first - b.first) * (a.first - c.first) >= 0)
    {
        ans = min(abs(a.first - c.first), abs(a.first - b.first));
    }
    if ((a.second - b.second) * (a.second - c.second) >= 0)
    {
        ans += min(abs(a.second - c.second), abs(a.second - b.second));
    }
