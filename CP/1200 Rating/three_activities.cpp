...
void solve()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        sort(all(arr[i]), [](const auto &a, const auto &b)
    ...
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 3; j++)
        {
            if (arr[0][i].second == arr[1][j].second)
                continue;
            for (int k = 0; k < 3; k++)
            {
                if (arr[0][i].second == arr[2][k].second || arr[1][j].second == arr[2][k].second)
                    continue;
                ans = max(ans, arr[0][i].first + arr[1][j].first + arr[2][k].first);
            }
        }
    }
