...
void towerOfHanoi(int &ans, vector<pair<int, int>> &moves, int n, int source, int mid, int dest)
{
    if (!n)
    {
        return;
    ...
    towerOfHanoi(ans, moves, n - 1, source, dest, mid);
    ans++;
    moves.push_back({source, dest});
    towerOfHanoi(ans, moves, n - 1, mid, source, dest);
}
...
int main()
{
    ...
    towerOfHanoi(ans, moves, n, 1, 2, 3);
